...
info_misinformation <- misinformation_panel %>%
  select(id_desinformacion, label_desinformacion) %>%
  distinct()

load('4-panel_data/misinformation/panel_interpolate_placebo.RData', verbose = TRUE)


### prepare placebo data

input_reg_placebo_temp <- panel_placebo %>%
  select(-date_desinformacion) %>%
  rename('date_desinformacion' = 'date_publicacion') %>%
...
## input reg of misinformation

load('4-panel_data/misinformation/panel_interpolate2.RData', verbose = TRUE)

input_reg <- misinformation_panel %>%
  filter(id_desinformacion %in% unique(panel_placebo$id_desinformacion)) %>%
...
input_reg_misinformation <- input_reg %>%
  filter(!is.na(days_since_misinformation_relative_fc)) %>%
  mutate(days_since_misinformation_relative_fc = as.factor(days_since_misinformation_relative_fc))%>%
  select(id_desinformacion, 
         ...
         approx_likes:growth_interactions, 
...
info_misinformation <- input_reg_misinformation %>%
  select(id_desinformacion, treatment, dsm_relative_fc, 
         label_desinformacion)%>% distinct()

input_reg_placebo <- input_reg_placebo_temp %>%
  left_join(info_misinformation, 'id_desinformacion') %>%
  select(-id_desinformacion)%>%
  mutate(treatment = ifelse(days_since_factcheck <= 0, 0, 1)) %>%
  select('id_desinformacion' = 'id_placebo', 
         'id_post_desinformacion'  = 'id_post_placebo', 
         id_factcheck, 
         treatment, 
         label_desinformacion,
         dsm_relative_fc,
  ...
  mutate(placebo = 1) %>%
  rbind(input_reg_misinformation)


save(input_reg_placebo, file = '5-analysis/1-input_data/misinformation/data_reg_misinformation_placebo.Rda')
